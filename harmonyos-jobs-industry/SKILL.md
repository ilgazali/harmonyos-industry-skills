---
name: harmonyos-jobs-industry
description: Use when building a HarmonyOS recruitment or job-search app - job card stacks, job listings, and the job-to-recruiter-to-company navigation flow.
---

# HarmonyOS Jobs and Recruitment Industry

Huawei HQ's verified feature catalog for this vertical: 5 features drawn
from `12_jobs`, backed by 5 evidence-based findings against HQ's own
published documentation. Feature IDs are prefixed `JOBS-`.
...
Everything below is what is specific to jobs and recruitment applications.
...
5. Check the card's **Pitfalls** and
   ...
   3 of 5 cards here correct at least one defect in HQ's
...
| [references/pitfalls.md](references/pitfalls.md) | the 5 findings, systemic ones first |
| `features/JOBS-*.md` | one card per feature: checklist, architecture, verified code, pitfalls |
...
- Only five features; the stacked job card and the three-step navigation flow are the substance.
- The in-app notification authorization card is the reusable pattern for asking before the system dialog.
