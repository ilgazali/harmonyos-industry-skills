---
name: harmonyos-food-industry
description: Use when building a HarmonyOS food-ordering, recipe or restaurant app - store location and routing, recipe templates, or custom pull-to-refresh lists.
---

# HarmonyOS Food and Dining Industry

Huawei HQ's verified feature catalog for this vertical: 6 features drawn
from `17_food`, backed by 30 evidence-based findings against HQ's own
published documentation. Feature IDs are prefixed `FOOD-`.
...
Everything below is what is specific to food and dining applications.
...
5. Check the card's **Pitfalls** and
   ...
   4 of 6 cards here correct at least one defect in HQ's
...
| File | What it holds |
|---|---|
| [references/feature-catalog.md](references/feature-catalog.md) | all 6 features in one table - start here |
| [references/api-map.md](references/api-map.md) | feature to kit / permission / API level, plus reverse indexes |
| [references/pitfalls.md](references/pitfalls.md) | the 30 findings, systemic ones first |
| `features/FOOD-*.md` | one card per feature: checklist, architecture, verified code, pitfalls |
...
- Six features, two of which are whole app templates - start from the template, not from a blank project.
- The custom pull-to-refresh card is the reference implementation reused across several other verticals.
