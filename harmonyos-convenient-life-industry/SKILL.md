---
name: harmonyos-convenient-life-industry
description: Use when building a HarmonyOS everyday-services app - government services, cinema seat selection, bill and to-do lists, calendars, password vaults, category pickers, or map-backed local services.
---

# HarmonyOS Convenient Life Industry

Huawei HQ's verified feature catalog for this vertical: 31 features drawn
from `02_convenient_life`, backed by 271 evidence-based findings against HQ's own
published documentation. Feature IDs are prefixed `LIFE-`.
...
Everything below is what is specific to convenient life applications.
...
5. Check the card's **Pitfalls** and
   ...
   30 of 31 cards here correct at least one defect in HQ's
...
| File | What it holds |
|---|---|
| [references/feature-catalog.md](references/feature-catalog.md) | all 31 features in one table - start here |
| [references/api-map.md](references/api-map.md) | feature to kit / permission / API level, plus reverse indexes |
| [references/pitfalls.md](references/pitfalls.md) | the 271 findings, systemic ones first |
| `features/LIFE-*.md` | one card per feature: checklist, architecture, verified code, pitfalls |
...
- This is the widest catalog in the package; search it before writing any custom picker or list interaction.
- Cascading pickers, date-time wheels and calendars all have verified templates here - do not rebuild them.
- Drag-to-reorder and pinch-to-switch gestures follow one shared pattern across the samples.
