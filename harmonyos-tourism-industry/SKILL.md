---
name: harmonyos-tourism-industry
description: Use when building a HarmonyOS travel, tourism or hotel app - destination maps and POIs, date-range selection, attraction audio guides, hotel orders and reviews, or traveller detail forms.
---

# HarmonyOS Tourism Industry

Huawei HQ's verified feature catalog for this vertical: 13 features drawn
from `09_tourism`, backed by 80 evidence-based findings against HQ's own
published documentation. Feature IDs are prefixed `TOUR-`.
...
Everything below is what is specific to tourism applications.
...
5. Check the card's **Pitfalls** and
   ...
   11 of 13 cards here correct at least one defect in HQ's
...
| File | What it holds |
|---|---|
| [references/feature-catalog.md](references/feature-catalog.md) | all 13 features in one table - start here |
| [references/api-map.md](references/api-map.md) | feature to kit / permission / API level, plus reverse indexes |
| [references/pitfalls.md](references/pitfalls.md) | the 80 findings, systemic ones first |
| `features/TOUR-*.md` | one card per feature: checklist, architecture, verified code, pitfalls |
...
- Map markers, reverse geocoding and long-press-to-drop are separate cards that compose into one screen.
- The location permission bubble card is the correct pre-request rationale pattern for the whole package.
- Date range picking and origin/destination swap are booking-flow staples with verified implementations.
