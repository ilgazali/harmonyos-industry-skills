---
name: harmonyos-children-education-industry
description: Use when building a HarmonyOS children's education or kids app - parent gates, screen-time limits, handwriting and drawing boards, puzzles and board games, vocabulary cards, or child location tracking.
---

# HarmonyOS Children's Education Industry

Huawei HQ's verified feature catalog for this vertical: 17 features drawn
from `08_children_education`, backed by 120 evidence-based findings against HQ's own
published documentation. Feature IDs are prefixed `KIDS-`.
...
Everything below is what is specific to children's education applications.
...
5. Check the card's **Pitfalls** and
   ...
   15 of 17 cards here correct at least one defect in HQ's
...
| File | What it holds |
|---|---|
| [references/feature-catalog.md](references/feature-catalog.md) | all 17 features in one table - start here |
| [references/api-map.md](references/api-map.md) | feature to kit / permission / API level, plus reverse indexes |
| [references/pitfalls.md](references/pitfalls.md) | the 120 findings, systemic ones first |
| `features/KIDS-*.md` | one card per feature: checklist, architecture, verified code, pitfalls |
...
- A parent gate and a screen-time limit are the two cards every kids app needs; start there.
- Drawing, handwriting and puzzle boards are all `Canvas`-based and share one input-handling pattern.
- Child location and distance-from-home alarms carry the heaviest permission burden in this vertical.
