---
name: harmonyos-finance-insurance-industry
description: Use when building a HarmonyOS finance, banking, insurance or investment app - stock charts, loan calculators, expenditure breakdowns, app lock, bank card entry, or secure custom keyboards.
---

# HarmonyOS Finance and Insurance Industry

Huawei HQ's verified feature catalog for this vertical: 11 features drawn
from `07_finance_insurance`, backed by 54 evidence-based findings against HQ's own
published documentation. Feature IDs are prefixed `FIN-`.
...
Everything below is what is specific to finance and insurance applications.
...
5. Check the card's **Pitfalls** and
   ...
   10 of 11 cards here correct at least one defect in HQ's
...
| File | What it holds |
|---|---|
| [references/feature-catalog.md](references/feature-catalog.md) | all 11 features in one table - start here |
| [references/api-map.md](references/api-map.md) | feature to kit / permission / API level, plus reverse indexes |
| [references/pitfalls.md](references/pitfalls.md) | the 54 findings, systemic ones first |
| `features/FIN-*.md` | one card per feature: checklist, architecture, verified code, pitfalls |
...
- Password entry uses a randomised-order custom keyboard; the system keyboard is not acceptable here.
- The pie chart card depends on the third-party mpchart library - check the dependency before copying it.
- App lock and card-number entry are the security-sensitive cards; their pitfalls are not optional reading.
