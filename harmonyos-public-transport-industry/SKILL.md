---
name: harmonyos-public-transport-industry
description: Use when building a HarmonyOS transit, ride-code or navigation app - real-time arrivals, home-screen widgets and shortcuts for a ride code, boarding reminders, ride history, or map controls.
---

# HarmonyOS Public Transport Industry

Huawei HQ's verified feature catalog for this vertical: 9 features drawn
from `06_public_transport`, backed by 52 evidence-based findings against HQ's own
published documentation. Feature IDs are prefixed `TRANS-`.
...
Everything below is what is specific to public transport applications.
...
5. Check the card's **Pitfalls** and
   ...
   9 of 9 cards here correct at least one defect in HQ's
...
| File | What it holds |
|---|---|
| [references/feature-catalog.md](references/feature-catalog.md) | all 9 features in one table - start here |
| [references/api-map.md](references/api-map.md) | feature to kit / permission / API level, plus reverse indexes |
| [references/pitfalls.md](references/pitfalls.md) | the 52 findings, systemic ones first |
| `features/TRANS-*.md` | one card per feature: checklist, architecture, verified code, pitfalls |
...
- The ride code is reached through a home-screen widget and a pinned shortcut - two separate mechanisms, both here.
- Boarding and alighting reminders use Notification Kit, not a foreground service.
- Map rotation lock, zoom stepping and locate-me are one card; do not implement them piecemeal.
