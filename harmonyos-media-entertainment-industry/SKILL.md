---
name: harmonyos-media-entertainment-industry
description: Use when building a HarmonyOS music, video, audio or streaming app - players and mini-players, karaoke lyrics, gesture controls, video compression and GIF export, playlists, or audio output switching.
---

# HarmonyOS Media and Entertainment Industry

Huawei HQ's verified feature catalog for this vertical: 43 features drawn
from `13_media_entertainment`, backed by 107 evidence-based findings against HQ's own
published documentation. Feature IDs are prefixed `MEDIA-`.
...
Everything below is what is specific to media and entertainment applications.
...
5. Check the card's **Pitfalls** and
   ...
   41 of 43 cards here correct at least one defect in HQ's
...
| File | What it holds |
|---|---|
| [references/feature-catalog.md](references/feature-catalog.md) | all 43 features in one table - start here |
| [references/api-map.md](references/api-map.md) | feature to kit / permission / API level, plus reverse indexes |
| [references/pitfalls.md](references/pitfalls.md) | the 107 findings, systemic ones first |
| `features/MEDIA-*.md` | one card per feature: checklist, architecture, verified code, pitfalls |
...
- Audio and video are two distinct architectures here; check which one a card belongs to before copying.
- The floating mini-player and the landscape/portrait switch both manipulate the window; they interact.
- Cellular-data reminders before playback are expected behaviour in this vertical.
