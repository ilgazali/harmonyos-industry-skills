---
name: harmonyos-photography-industry
description: Use when building a HarmonyOS camera, photo-editing or imaging app - capture and recording, filters and cut-outs, collages and stickers, compression and format conversion, face detection, or GIF creation.
---

# HarmonyOS Photography and Imaging Industry

Huawei HQ's verified feature catalog for this vertical: 32 features drawn
from `18_photography`, backed by 95 evidence-based findings against HQ's own
published documentation. Feature IDs are prefixed `PHOTO-`.
...
Everything below is what is specific to photography and imaging applications.
...
5. Check the card's **Pitfalls** and
   ...
   30 of 32 cards here correct at least one defect in HQ's
...
| File | What it holds |
|---|---|
| [references/feature-catalog.md](references/feature-catalog.md) | all 32 features in one table - start here |
| [references/api-map.md](references/api-map.md) | feature to kit / permission / API level, plus reverse indexes |
| [references/pitfalls.md](references/pitfalls.md) | the 95 findings, systemic ones first |
| `features/PHOTO-*.md` | one card per feature: checklist, architecture, verified code, pitfalls |
...
- Prefer the permission-free paths: `SaveButton` for saving and `PhotoViewPicker` for picking. Nine samples here declare media permissions they never use.
- Recorder samples share a file-descriptor lifecycle defect; read the systemic pitfalls before copying any capture code.
- Composition has two routes - fixed stencils on a `PixelMap`, or a real `Canvas` for free placement. Pick deliberately.
