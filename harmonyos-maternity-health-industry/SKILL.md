---
name: harmonyos-maternity-health-industry
description: Use when building a HarmonyOS maternity, baby-care or child-health app - growth record timelines and growth curve charts.
---

# HarmonyOS Maternity and Child Health Industry

Huawei HQ's verified feature catalog for this vertical: 5 features drawn
from `10_maternity_health`, backed by 33 evidence-based findings against HQ's own
published documentation. Feature IDs are prefixed `MAT-`.
...
Everything below is what is specific to maternity and child health applications.
...
5. Check the card's **Pitfalls** and
   ...
   4 of 5 cards here correct at least one defect in HQ's
...
| File | What it holds |
|---|---|
| [references/feature-catalog.md](references/feature-catalog.md) | all 5 features in one table - start here |
| [references/api-map.md](references/api-map.md) | feature to kit / permission / API level, plus reverse indexes |
| [references/pitfalls.md](references/pitfalls.md) | the 33 findings, systemic ones first |
| `features/MAT-*.md` | one card per feature: checklist, architecture, verified code, pitfalls |
...
- The smallest catalog in the package - five features, all of them chart or timeline work.
- Growth curves are `Canvas` drawings with medically meaningful axes; do not substitute a generic chart.
